...
def arabic_multiplication(num1, num2):
    ...
    int_martix = [[i * j for i in num1_list] for j in num2_list]
    str_martix = [map(convert_to_str, int_martix[i])
                  for i in range(len(int_martix))]

    martix = [[int(str_martix[i][j][z]) for j in range(len(str_martix[i]))]
              for i in range(len(str_martix)) for z in range(2)]
